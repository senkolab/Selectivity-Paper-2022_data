...
def findSingleIonAverage(Counts, BG, BGStd, PlotIonCounts=False):
    CountsNonBG = Counts[Counts > BG + 10*BGStd]
...
data_files = "Load-Eff-Spect_Ba138_100uJ_8uW.*"
new_file = "Load-Eff-Spect_Ba138_100uJ_8uW_Sanitized"
data_files = glob.glob(data_files)
# new_file = Sanitize_SelectivityData(data_files, new_file)
new_file = "Load-Eff-Spect_Ba138_100uJ_8uW_Sanitized.txt"
...
IonCounts1, IonCountsStd = findSingleIonAverage(raw_data[:,5], BG, BGstd, True)
...
ax1.axvline(39, color='k', label='Spectrum Peak')
ax2 = ax1.twinx()
...
ax2.text(60, 18, WriteString, fontsize=14)
